...
All files delivered to Ben must be **version-stamped in the filename** — e.g. `v1.10_description.sh`, `HANDOFF_v1.10.md`. Ben's Downloads folder has many files from many projects; ambiguous names get lost.
...
### Cross-chat fetch staleness (lesson from v1.10)
A Claude session's `web_fetch` of `raw.githubusercontent.com` can silently return **stale cached content** — not from GitHub's CDN, but from the assistant's own tool layer. Symptoms: fetched `HANDOFF.md`/`index.html` show an old version number or even completely wrong content (e.g. a totally different app), while `git log` on-device shows the correct, pushed, tagged state. Retrying the same raw URL, a SHA-pinned URL, or a jsDelivr mirror URL may all fail or return the same stale content, because some of those calls get blocked outright as "not previously discovered via search."

**This is not fixable from inside the chat.** There's no cache-bust, header, or retry pattern that reliably forces a fresh read once this happens.

**Resolution:** don't keep retrying fetch variants. Ask Ben to run `git log --oneline -5` (confirms repo state) and then paste `cat HANDOFF.md` / `cat index.html` directly into the chat. This bypasses the fetch tool entirely and is the ground truth.
...
If the fetched version number or content doesn't match what I tell you, don't
retry the fetch — ask me to run `git log --oneline -5` and paste
`cat HANDOFF.md` / `cat index.html` directly instead.
